  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-gray-100 pb-5">
        <div>
          <h2 className="text-2xl font-sans font-bold text-slate-900 tracking-tight">Vehicle Registry</h2>
          <p className="text-sm text-slate-500 mt-1">Manage transport equipment profiles, fleet status, and cargo capacities.</p>
        </div>
      ...
      {!canEdit && (
        <div className="bg-blue-50 text-blue-800 px-4 py-3 border border-blue-100 rounded-xl flex items-center gap-3 text-xs">
          <Info size={16} />
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full text-sm bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:ring-1 focus:ring-teal-500 text-slate-700"
          />
        </div>
        ...
        <div className="w-full md:w-48">
          <select
            ...
            className="w-full text-sm bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-teal-500 text-slate-700"
          >
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-sm text-slate-600">
            <thead className="bg-slate-50 border-b border-slate-100 font-sans font-semibold text-slate-500 text-xs">
              <tr>
                <th className="py-3.5 px-5">
                  <button onClick={() => handleSort('registrationNumber')} className="flex items-center gap-1.5 hover:text-slate-800 cursor-pointer">
                ...
                <th className="py-3.5 px-5">
                  <button onClick={() => handleSort('maxLoadCapacity')} className="flex items-center gap-1.5 hover:text-slate-800 cursor-pointer">
                    <span>CAPACITY</span>
                    <ArrowUpDown size={12} />
                  </button>
                </th>
                <th className="py-3.5 px-5">
                  <button onClick={() => handleSort('odometer')} className="flex items-center gap-1.5 hover:text-slate-800 cursor-pointer">
                    <span>ODOMETER</span>
                    <ArrowUpDown size={12} />
                  </button>
                </th>
                <th className="py-3.5 px-5">
                  <button onClick={() => handleSort('acquisitionCost')} className="flex items-center gap-1.5 hover:text-slate-800 cursor-pointer">
                    <span>ACQ. COST</span>
                    <ArrowUpDown size={12} />
                  </button>
                </th>
                ...
                sortedVehicles.map(v => (
                  <tr key={v.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-5">
                      ...
                      <div className="text-xs text-slate-400 font-mono mt-0.5">{v.type}</div>
                    </td>
                      ...
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-red-500 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            ...
            {/* Modal Error Message */}
            {modalError && (
              <div className="mx-6 mt-4 bg-red-50 border border-red-100 text-red-700 p-3.5 rounded-xl flex items-start gap-2.5 text-xs">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-1">
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Registration #</label>
                  <input
                    ...
                    className="w-full text-sm bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-teal-500 text-slate-800 font-mono uppercase"
                ...
                <div className="col-span-1">
                  ...
                  <input
                    ...
                    className="w-full text-sm bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-teal-500 text-slate-800"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* Type */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Equipment Category</label>
                  <select
                ...
                {/* Max Load capacity */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Max Load Capacity (kg)</label>
                  <input
                    id="modal-vehicle-capacity"
                    type="number"
                    value={maxLoad || ''}
                    onChange={(e) => setMaxLoad(Number(e.target.value))}
                    className="w-full text-sm bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-teal-500 text-slate-800 font-mono"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* Acquisition cost */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Acquisition Cost (INR)</label>
                  <input
